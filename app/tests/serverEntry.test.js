...
describe('rendering to string', () => {
  const url = 'http://www.example.com/user/tom'
  const assets = {
    main: {
      js: 'xxx',
      css: 'yyy',
  ...
  describe('match callback', () => {
    ...
    describe('match found', () => {
      const consoleErrorFunc = console.error // eslint-disable-line no-console
      const dummyHtml = '<html>TADA!</html>'
      ...
      afterEach(() => {
        console.error = consoleErrorFunc // eslint-disable-line no-console
      })
      ...
      describe('error while rendering', () => {
        ...
        it('should call the callback with the error', () => {
          ...
          return waitFor(() => callback.mock.calls.length > 0).then(() => {
            const args = callback.mock.calls[0]
      ...
      describe('error 404', () => {
        it('should call the callback with null as html', () => {
          const renderProps = {
            routes: [{ name: 'aaa' }, { name: 'notfound' }, { name: 'bbb' }],
          }
